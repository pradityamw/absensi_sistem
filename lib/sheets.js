...
const GoogleSheetsSync = {
     ...
     */
    async saveAttendance(day, presentObjects, masterStudents, isSingleEdit = false) {
        const masterNames = masterStudents.map(s => typeof s === 'string' ? s : `${s.firstName} ${s.lastName}`.trim());
        const method = this.config.method;

        if (method === 'simulated') {
            return this._saveSimulatedData(day, presentObjects, masterNames, isSingleEdit);
        }
        ...
        if (method === 'direct-api') {
            return this._saveDirectApiData(day, presentObjects, masterNames, isSingleEdit);
        }
    ...
    _saveSimulatedData(day, presentObjects, masterStudents, isSingleEdit = false) {
        const key = `sim_attendance_${this.config.sheetName.replace(/\s+/g, '_')}`;
        ...
        currentData.forEach(row => {
            ...
            if (masterSet.has(nameLower)) {
                if (presentMap.has(nameLower)) {
                    ...
                    // Check if there is already a positive value in this cell (from a previous paste)
                    const existingCell = row[day.toString()];
                    const hasPositiveVal = !isSingleEdit && existingCell && typeof existingCell === 'object' && parseFloat(existingCell.value) > 0;
    ...
    async _saveDirectApiData(day, presentObjects, masterStudents, isSingleEdit = false) {
        if (!this.config.accessToken) {
        ...
        studentRows.forEach((row, index) => {
            ...
            // Check if there is already a positive value in the existing row's cell
            const existingCell = row[day.toString()];
            const hasPositiveVal = existingCell && typeof existingCell === 'object' && parseFloat(existingCell.value) > 0;
            
            const duration = isPresent 
                ? presentMap.get(nameLower).jam 
                : ((!isSingleEdit && hasPositiveVal) ? parseFloat(existingCell.value) : 0);
